...
def axis_walk_save(sess, model, images, step=0, directory="default"):
  x_reconstruct = model.axis_walk(sess, images)
  ...
  for i in range(len(images)):
    ...
    imsave("{0}/org_{1}.png".format(flags.reconstr,i),org_img)
    for j in range(model.z_dim):
        reconstr_img = x_reconstruct[j][i].reshape(64, 64)
  ...
  x_reconstruct = map(lambda x : np.expand_dims(x, len(x.shape)), x_reconstruct)
  x_reconstruct = np.concatenate(x_reconstruct + [images], axis=-1)
...
#    autoencoded_sample_save(manager=manager_alt, flags=flags, directory="default_autoencded_data", sess=sess, model=model)
    for i in range(1):
        images = manager.get_random_images(640)
