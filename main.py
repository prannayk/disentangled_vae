...
def axis_walk_save(sess, model, images, step=0, directory="default"):
  ...
  x_reconstruct = x_reconstruct.reshape([2*model.K, model.z_dim, images.shape[0],64,64])
  print(x_reconstruct.shape)
...
#  x_reconstruct = np.swapaxes(x_reconstruct, 3,4)
  x_reconstruct = x_reconstruct.reshape([model.z_dim, images.shape[0], 64,2*model.K*64])
  ...
  x_reconstruct = np.swapaxes(x_reconstruct, 0, 1)
  x_reconstruct = np.swapaxes(x_reconstruct, 2, 3)
  x_reconstruct = x_reconstruct.reshape([images.shape[0],model.z_dim, 2*model.K, 64,64])
  print(x_reconstruct.shape)
